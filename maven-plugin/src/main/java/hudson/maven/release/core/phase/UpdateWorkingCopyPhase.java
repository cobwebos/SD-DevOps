...
import hudson.maven.release.core.utils.JenkinsScmLogger;
 ...
 * @version $Id: UpdateWorkingCopyPhase.java 1126667 2011-05-23 19:08:15Z oching
 *          $
 */
public class UpdateWorkingCopyPhase extends AbstractReleasePhase {
	...
	@Override
	public ReleaseResult execute(ReleaseDescriptor releaseDescriptor, Settings settings, List reactorProjects) throws ReleaseExecutionException,
		...
		// CONTINUUM-2628
		// if git ssh, use credentials specified in scm url if present.
		// otherwise, use the scm credentials of project
		...
		if (providerType.contains("GIT") && scmSpecificUrl.startsWith(GitScmProviderRepository.PROTOCOL_SSH)) {
		...
		try {
			...
			if (providerRepo instanceof GitScmProviderRepository) {
				String branchName = GitBranchCommand
						.getCurrentBranch(new JenkinsScmLogger(getLogger()), (GitScmProviderRepository) providerRepo, workingDirSet);
